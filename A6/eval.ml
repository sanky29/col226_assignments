...
let rec ans_to_string (vars:variable list) ans =
	...
	begin
		match h2 with
		| Var x -> if (String.get x 0 = '_' || String.compare x h1 = 0) then "" 
				   else h1^"="^x^","^(ans_to_string t1 t2)  
...
(* let kb = [|(("edge",2),[|Fact (("edge",2),[Const "a";Const "b"]);
							  Fact (("edge",2),[Const "b";Const "c"]);
							  Fact (("edge",2),[Const "c";Const "d"]);
							  Fact (("edge",2),[Const "c";Const "e"]);|]);
		("path",2),[|Fact (("path",2),[Var "_X";Var "_X"]);
				Rule ((("path",2),[Var "_X";Var "_Y"]),[(("edge",2),[Var "_X";Var "_Z"]);(("path",2),[Var "_Z";Var "_Y"])])|]|];;
let queryl = [("path",2),[Var "X";Var "Y"]];;
let (stack:('a list ref)) = ref [];;
	let (subs:('b list ref)) = ref [];;
	let sub_stack = ref [0];;
	let depth = ref 0;;
	let query = ref (Array.of_list queryl);;
	let variables = var_afl queryl;;
	let visited = ref (Array.make (List.length queryl) (-1));;
	let got = ref false;;
	let id = ref 0;;
